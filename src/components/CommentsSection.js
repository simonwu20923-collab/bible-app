...
if (typeof document !== 'undefined' && !document.getElementById('comments-style')) {
  ...
  s.textContent = `
    ...
    .thread-line { width:2px; background:rgba(124,58,237,0.25); border-radius:1px; transition:background 0.15s; min-height:100%; }
    .thread-line-wrap:hover .thread-line { background:rgba(124,58,237,0.7); }
    .collapse-btn { background:none; border:1px solid rgba(255,255,255,0.15); border-radius:3px; color:#7878a8;
    ...
    .collapse-btn:hover { border-color:#7c3aed; color:#c0c0ff; }
  `;
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 5, flexWrap: 'wrap', marginTop: 5 }}>
          ...
          <div key={emoji} className="reaction-wrapper" style={{ position: 'relative', display: 'inline-block' }}>
            <button onClick={() => onReact(comment, emoji)} title={names.join(', ')}
              style={{
                ...
                color: '#c0c0e0', fontSize: 13, display: 'flex', alignItems: 'center', gap: 4,
              }}>
              {emoji} <span style={{ fontWeight: 600 }}>{names.length}</span>
            </button>
            <div className="reaction-tooltip" style={{
              position: 'absolute', bottom: 'calc(100% + 6px)', left: '50%', transform: 'translateX(-50%)',
              background: '#0d0d1f', border: '1px solid #2a2a4a', borderRadius: 6, padding: '4px 8px',
              whiteSpace: 'nowrap', fontSize: 11, color: '#c0c0e0', zIndex: 200,
      ...
      <div style={{ position: 'relative' }}>
        <button onClick={() => setPickerOpen(p => !p)}
          style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#7878a8', fontSize: 12, padding: '1px 5px', display: 'flex', alignItems: 'center', gap: 3 }}>
          🙂 {reactLabel}
        </button>
        {pickerOpen && (
          <div style={{ position: 'absolute', bottom: '100%', left: 0, zIndex: 100, background: '#1e1e3a', border: '1px solid #2a2a4a', borderRadius: 8, padding: '6px 8px', display: 'flex', gap: 4, boxShadow: '0 4px 20px rgba(0,0,0,0.5)' }}>
            {EMOJIS.map(e => (
      ...
      <div style={{
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
            <span style={{ color: '#e0e0ff', fontWeight: 600, fontSize: 13 }}>{comment.name}</span>
            <span style={{ color: '#5a5a7a', fontSize: 11 }}>{timeAgo(comment.created_at)}</span>
            {collapsed && (
          ...
          {!collapsed && (
            <>
              <p style={{ color: '#c0c0e0', fontSize: 14, lineHeight: 1.55, margin: '4px 0 0', wordBreak: 'break-word' }}>
                {comment.text}
              </p>
              ...
              <div style={{ display: 'flex', gap: 12, marginTop: 5, alignItems: 'center' }}>
                <button onClick={() => setReplyOpen(r => !r)}
                  style={{ background: 'none', border: 'none', color: '#7878a8', fontSize: 12, cursor: 'pointer', padding: 0, fontWeight: 600 }}>
                  ↩ {replyLabel}
                </button>
                {children.length > 0 && (
                  <button onClick={() => setChildrenCollapsed(c => !c)}
                    style={{ background: 'none', border: 'none', color: '#7878a8', fontSize: 12, cursor: 'pointer', padding: 0 }}>
                ...
                <div style={{ marginTop: 8, display: 'flex', gap: 8 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <textarea value={replyText} onChange={e => setReplyText(e.target.value)}
                      ...
                      rows={2} autoFocus
                      style={{ width: '100%', background: '#1a1a2e', border: '1px solid #2a2a4a', borderRadius: 8, color: '#e0e0ff', fontSize: 13, padding: '7px 10px', resize: 'none', boxSizing: 'border-box' }} />
                    <div style={{ display: 'flex', gap: 6, marginTop: 4 }}>
                      <button onClick={submitReply} disabled={submitting || !replyText.trim()}
                        style={{ background: '#7c3aed', border: 'none', borderRadius: 6, color: '#fff', fontSize: 12, fontWeight: 600, padding: '4px 12px', cursor: 'pointer', opacity: submitting || !replyText.trim() ? 0.5 : 1 }}>
                        {submitLabel}
                      </button>
                      <button onClick={() => { setReplyOpen(false); setReplyText(''); }}
                        style={{ background: 'none', border: 'none', color: '#7878a8', fontSize: 12, cursor: 'pointer' }}>
                        {cancelLabel}
                      </button>
                    </div>
                  </div>
  ...
  return (
    <div style={{ marginTop: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
        <span style={{ color: '#a0a0c0', fontWeight: 600, fontSize: 14 }}>
          {t.title} {comments.length > 0 && `(${comments.length})`}
        </span>
        {currentName && (
          <button onClick={() => setShowForm(f => !f)}
            style={{ background: showForm ? '#2a2a4a' : '#7c3aed', border: 'none', borderRadius: 8, color: '#fff', fontSize: 12, padding: '6px 12px', cursor: 'pointer', fontWeight: 600 }}>
      ...
      {showForm && currentName && (
        <div style={{ display: 'flex', gap: 10, marginBottom: 16, background: '#12122a', borderRadius: 10, padding: '10px 12px', border: '1px solid #2a2a4a' }}>
          <Avatar name={currentName} />
          <div style={{ flex: 1 }}>
            <textarea value={text} onChange={e => setText(e.target.value)} placeholder={t.placeholder} rows={3} autoFocus
              style={{ width: '100%', background: 'transparent', border: 'none', color: '#e0e0ff', fontSize: 14, resize: 'none', outline: 'none', boxSizing: 'border-box', lineHeight: 1.5 }} />
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
              <button onClick={() => { setShowForm(false); setText(''); }}
                style={{ background: 'none', border: 'none', color: '#7878a8', fontSize: 13, cursor: 'pointer' }}>{t.cancel}</button>
              <button onClick={submitComment} disabled={submitting || !text.trim()}
                style={{ background: '#7c3aed', border: 'none', borderRadius: 6, color: '#fff', fontSize: 13, fontWeight: 600, padding: '5px 16px', cursor: 'pointer', opacity: submitting || !text.trim() ? 0.5 : 1 }}>
                {t.submit}
              </button>
            </div>
          </div>
      ...
      {!currentName && <p style={{ color: '#5a5a7a', fontSize: 13, fontStyle: 'italic' }}>{t.loginPrompt}</p>}

      {loading ? (
        <div style={{ color: '#5a5a7a', fontSize: 13, padding: '12px 0' }}>Loading...</div>
      ) : topLevel.length === 0 ? (
        <div style={{ color: '#5a5a7a', fontSize: 13, padding: '12px 0' }}>{t.noComments}</div>
      ) : (
        <div>
...
export function HomeThreadCard({ threadComments, lang = 'en', onNavigate }) {
  ...
  return (
    <div style={{ background: '#0f0f20', borderRadius: 10, padding: '2px 12px 8px', border: '1px solid #1e1e3a', marginBottom: 10 }}>
      {topLevel.map(comment => (
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 6, paddingLeft: 4 }}>
        {hiddenCount > 0 && (
          <span style={{ color: '#5a5a7a', fontSize: 12 }}>{hiddenCount} more {replyLabel}</span>
        )}
