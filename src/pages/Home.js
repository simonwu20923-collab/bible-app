...
export default function Home({ lang = 'en' }) {
  ...
  const ui = {
    en: { title:'Church in Cerritos', sub:'Bible Reading Tracker 2026', namePlaceholder:'Enter your name...', start:'Start →', streak:'Day Streak 🔥', fullDays:'Full Days ✓', daysRead:'Days Read', continueReading:'📖 Continue Reading', next:'Next:', todayReading:"📅 Today's Reading", goToDate:'Go to current date', viewSchedule:'🗓 View Schedule', calList:'Calendar & List', thisWeek:'🏆 This Week', allTime:'⭐ All Time', noReadings:'No readings this week yet', full:'full', days:'days', recentComments:'💬 Recent Comments', loading:'Loading...' },
    es: { title:'Iglesia en Cerritos', sub:'Registro de Lectura Bíblica 2026', namePlaceholder:'Ingresa tu nombre...', start:'Comenzar →', streak:'Racha 🔥', fullDays:'Días Completos ✓', daysRead:'Días Leídos', continueReading:'📖 Continuar Leyendo', next:'Siguiente:', todayReading:'📅 Lectura de Hoy', goToDate:'Ir a la fecha actual', viewSchedule:'🗓 Ver Horario', calList:'Calendario y Lista', thisWeek:'🏆 Esta Semana', allTime:'⭐ Todo el Tiempo', noReadings:'Sin lecturas esta semana', full:'completo', days:'días', recentComments:'💬 Comentarios Recientes', loading:'Cargando...' },
    zh: { title:'瑟瑞托斯召會', sub:'2026年聖經閱讀記錄', namePlaceholder:'輸入你的名字...', start:'開始 →', streak:'連續天數 🔥', fullDays:'完整天數 ✓', daysRead:'已讀天數', continueReading:'📖 繼續閱讀', next:'下一個：', todayReading:'📅 今日閱讀', goToDate:'前往今天', viewSchedule:'🗓 閱讀計劃', calList:'日曆與列表', thisWeek:'🏆 本週', allTime:'⭐ 總排行', noReadings:'本週還沒有閱讀記錄', full:'全部', days:'天', recentComments:'💬 最近留言', loading:'載入中...' },
    sc: { title:'塞里托斯召会', sub:'2026年圣经阅读记录', namePlaceholder:'输入你的名字...', start:'开始 →', streak:'连续天数 🔥', fullDays:'完整天数 ✓', daysRead:'已读天数', continueReading:'📖 继续阅读', next:'下一个：', todayReading:'📅 今日阅读', goToDate:'前往今天', viewSchedule:'🗓 阅读计划', calList:'日历与列表', thisWeek:'🏆 本周', allTime:'⭐ 总排行', noReadings:'本周还没有阅读记录', full:'全部', days:'天', recentComments:'💬 最近留言', loading:'加载中...' },
  };
  ...
  async function loadRecentComments() {
    const { data } = await supabase.from('comments').select('name,text,date,created_at').order('created_at', { ascending: false }).limit(5);
    if (data) setRecentComments(data);
  }

  async function loadMyStats(n) {
    const { data } = await supabase.from('checkins').select('date,portion').ilike('name', n);
    if (!data) return;
    const byDate = {};
    data.forEach(r => {
  ...
  function saveName() {
    if (nameInput.trim()) { localStorage.setItem('bibleAppName', nameInput.trim()); setName(nameInput.trim()); }
  }
  ...
  function timeAgo(ts) {
    ...
    if (m < 1) return lang === 'en' ? 'just now' : lang === 'es' ? 'ahora' : '刚刚';
    if (m < 60) return lang === 'en' ? `${m}m ago` : lang === 'es' ? `hace ${m}m` : `${m}分钟前`;
    const h = Math.floor(m / 60);
    if (h < 24) return lang === 'en' ? `${h}h ago` : lang === 'es' ? `hace ${h}h` : `${h}小时前`;
    return lang === 'en' ? `${Math.floor(h/24)}d ago` : lang === 'es' ? `hace ${Math.floor(h/24)}d` : `${Math.floor(h/24)}天前`;
  }
    ...
    <div className="page">
      {stats && (
        <div className="stats-row">
          <div className="stat-card"><div className="stat-number">{stats.streak}</div><div className="stat-label">{t.streak}</div></div>
          <div className="stat-card"><div className="stat-number">{stats.fullDays}</div><div className="stat-label">{t.fullDays}</div></div>
          <div className="stat-card"><div className="stat-number">{stats.totalDays}</div><div className="stat-label">{t.daysRead}</div></div>
        </div>
      )}
      <div className="home-buttons">
        {nextDate && (
          <button className="home-btn home-btn-primary"
            onClick={() => navigate(nextDate === new Date().toISOString().split('T')[0] ? '/reading' : `/reading?date=${nextDate}`)}>
      ...
      {loading ? <div className="loading">{t.loading}</div> : (
        <div className="leaderboards">
          <div className="leaderboard-card">
            ...
            {topWeek.length === 0 ? <div className="lb-empty">{t.noReadings}</div>
              : topWeek.map((r, i) => (
      ...
      {recentComments.length > 0 && (
        <div className="recent-comments-section">
          ...
          {recentComments.map((c, i) => (
            <div className="comment" key={i} onClick={() => navigate(`/reading?date=${c.date}`)} style={{ cursor: 'pointer' }}>
